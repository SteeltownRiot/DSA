...
assault = dfw.loc[ward_incidents['Offense'] == 'ASSAULT', ['Month', 'Ward', 'Incidents']]
assault_jul = assault.loc[assault['Month'] == 7, ['Ward', 'Incidents']]
assault_apr = assault.loc[assault['Month'] == 4, ['Ward', 'Incidents']]
battery = dfw.loc[ward_incidents['Offense'] == 'BATTER', ['Month', 'Ward', 'Incidents']]
...
myy = 100*(np.array(assault_jul['Incidents']) - np.array(assault_apr['Incidents']))/np.array(assault_apr['Incidents'])
...
ax.scatter(myx, myy, s = 1/50*(np.array(battery_jul['Incidents']) + np.array(assault_jul['Incidents'])))
